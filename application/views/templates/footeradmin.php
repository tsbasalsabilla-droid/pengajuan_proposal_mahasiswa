...
<script>
    // PINDAHKAN FUNGSI UPDATE STATUS KE LUAR $(document).ready AGAR GLOBAL
    function updateStatus(status) {
        ...
        $.ajax({
            ...
            success: function(res) {
                console.log("Respon Server:", res); // CEK: Apakah server balas {status: true}?
                if (res.status) {
                    $('#statusModal').modal('hide');
                    
                    // Reload semua DataTable yang mungkin ada dengan ID yang benar
                    ...
                    if ($.fn.DataTable.isDataTable('#table-proposal-dosen')) {
                        $('#table-proposal-dosen').DataTable().ajax.reload(null, false);
                    }
                    
                    alert('Status berhasil diperbarui!');
                } else {
                    alert("Gagal update di database: " + res.message);
                }
            },
            error: function(xhr) {
        ...
        function updateStatus(status) {
            ...
            $.ajax({
                ...
                success: function(res) {
                    if (res.status) {
                        $('#statusModal').modal('hide');
                        // Reload tabel agar status berubah otomatis
                        $('#datatable-daftar').DataTable().ajax.reload(null, false);
                    }
                },
...
$(document).on('click', '.btn-delete-dosen', function(e) {
    ...
    // Tambahkan kode berikut di dalam ready
    $('#datatable-datasiswa').DataTable({
        "ajax": {
            "url": "<?= base_url('data/getdatasiswa'); ?>",
            "type": "POST",
